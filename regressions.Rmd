...
```{r}
### Open Bol points
luc_bol_fa <- st_read(luc_bol_fa,
                      "bol_gridpts_for_regression.shp")
...
```{r}
### Make corr table
cor_table_bol <- flattenSquareMatrix(cor.prob(bol_dat[, c(3:11, 13:15, 18, 19)]))

### Look at vars with abs(cor.prob) > 0.66 and pvalue < 0.05
cor_table_bol <- subset(cor_table_bol, 
                        abs(cor) > 0.66 & p <= 0.05)

### Distance to roads and distance to cities highly correlated (coeff = 0.7177882)
### Poverty and soil moisture are highly correlated (coeff = 0.7959958)
### Poverty and longitude (-0.7807923)
### Soil and longitude (-0.8935180)
### Precip and latitude (0.8377322)
### Lon and lat (-0.7334816)

### Drop soil moisture (poverty, longitude), distance to cities (distance to roads), precip (latitude), longitude
...
### Drop geometry
bol_dat_simp <- bol_dat_red
bol_dat_simp$geometry <- NULL

### Drop incomplete rows
bol_dat_simp <- bol_dat_simp[complete.cases(bol_dat_simp), ]

### Drop rows where prot_status = 0 (??)
bol_dat_simp <- bol_dat_simp %>%
  filter(., !prot_status == "0")
```
...
```{r}
### Make corr table
cor_table_bol <- flattenSquareMatrix(cor.prob(bol_dat_m2[, c(3:8, 12:13, 16)]))
...
### Highly correlated variables
### Distance to illegal mines and longitude (0.9861674)
### Distance to illegal mines and latitude (-0.8341533)
### Longitude and latitude (-0.7334816)

### Drop lon and lat

### Make updated dataset
bol_dat_red_m2 <- bol_dat_m2 %>%
  dplyr::select(-lon, -lat)

bol_simp_m2 <- bol_dat_red_m2
bol_simp_m2$geometry <- NULL

### Drop incomplete rows
bol_dat_comp_m2 <- bol_simp_m2[complete.cases(bol_simp_m2), ]
```
...
```{r}
### Drop geometry
bol_dat_2$geometry <- NULL

### Make corr table
cor_table_bol <- flattenSquareMatrix(cor.prob(bol_dat_2[, c(3:11, 13:21, 25, 26, 28)]))
...
### Highly correlated variables
### Distance to roads and cities (0.7177882)
### Poverty and soil moisture (0.7959958)
### Poverty and distance to illegal mining (-0.7784845)
### Soil moisture and distance to illegal mining (-0.8533168)
### Poverty and longitude (-0.7807923)
### Soil moisture and longitude (-0.8935180)
### Distance to illegal mining and longitude (0.9861674)
### Precip and latitude (0.8377322)
### Illegal mining and latitude (-0.8341533)
### Lon and lat (-0.7334816)

### Drop lon, lat, soil moisture (illegal mining), distance to cities (distance to roads), poverty (illegal mining)

### Make updated dataset
bol_dat_red_m3 <- bol_dat_2 %>%
  dplyr::select(-lon, -lat, -soil, 
                -cities, -poverty)

### Drop incomplete rows
bol_dat_comp_m3 <- bol_dat_red_m3[complete.cases(bol_dat_red_m3), ]

### Drop rows where prot_status = 0 (??)
bol_dat_comp_m3 <- bol_dat_comp_m3 %>%
  filter(., !prot_status == "0")
```
...
fit_bol_m3 <- glm(trans_rc ~ aspect + slope + elev +
                    mines + roads + rivers +
                    popdens + ppt + prot_status +
                    crop_suit + 
                    dist_ag + dist_fire +
                    fire_dens + field_res +
                    ill_mine + tourism + pes +
                    bol_cows,
                  data = bol_dat_comp_m3,
                  family = binomial(link = "logit"))
...
* poverty rate, protection status (all), slope, aspect, elevation, distance to roads, distance to legal mines, latitude
...
bol_dat_4 <- bol_dat_2 %>%
  dplyr::select(uid, poverty, prot_status, slope, aspect,
                elev, roads, mines, lat,
                dist_fire, fire_dens, tourism, pes, bol_cows,
                trans_rc)

### Make corr table
cor_table_bol <- flattenSquareMatrix(cor.prob(bol_dat_4[, c(2, 4:12, 14)]))
...
### Drop incomplete rows
bol_dat_comp_m4 <- bol_dat_4[complete.cases(bol_dat_4), ]

### Drop rows where prot_status = 0 (??)
bol_dat_comp_m4 <- bol_dat_comp_m4 %>%
  filter(., !prot_status == "0")
```
