...
#### 8/18
Resample file so it matches extent and resolution of land use/land cover map. There was an issue with the file, so restarting the process. Opened copy of AreasUrbanizadas_MunicipiosAcima100k_porMunicipio.shp in ArcMap and manually removed cities that are very far from Jamanxim. Then made bra_dist_urb2 in ArcMap. Now need to clip and resample it.
```{r}
### Open city shp to reproject
brazil_urb <- st_read("D:/dinamica/brazil_dinamica_data/areas_urbanizadas_do_brasil_2005_ibge/AreasUrbanizadas_MunicipiosAcima100k_porMunicipio_reduced.shp")

### Project to CRS of rest of files and write out
brazil_urb <- st_transform(brazil_urb, crs = 102033) %>%
  st_write(., "D:/dinamica/processed_layers/areas_urbanizadas_reduced.shp")

### Now use Euclidean distance to make raster using this output (in ArcGIS)


### Need to redo calculation of distance to urban areas for Jamanxim area

# ### Resample to match jm_2008
# j_urb_resample <- raster::resample(jaman_urb, jm_2008,
#                                   "bilinear")
