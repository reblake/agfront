...
### Drop cols that are diff
# min_li <- min_li %>%
#   dplyr::select(., -shape_Leng, -metodoexpl)
min_po <- min_po %>%
  dplyr::select(., -shape_Leng,
                -shape_Area, -hectares)

### Combine
all_min <- rbind(min_pt, min_po)
# min_comb <- st_combine(min_pt, min_po)
...
### Bolivia only needs polygons, so write that out separately
st_write(min_po,
