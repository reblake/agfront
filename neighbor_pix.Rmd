...
```{r}
### Open raster
peru_map <- raster("C:/Users/Katherine Siegel/Documents/SESYNC/dinamica/remote_sensing/classi_per_dry_2008_102033.tif")

### Open raster-->pt from ArcMap
peru_pts <- st_read("C:/Users/Katherine Siegel/Documents/SESYNC/dinamica/remote_sensing/peru_pts.shp")

### Function to calculate % neighbors of different types
percent_different_neighbors <- function(cell) {
  # pull neighboring values
  # see ?adjacent for the arguments used here
  x <- getValues(peru_map)[adjacent(peru_map, 
                                    cells = cell, 
                                    directions = 8, 
                                    pairs = F, 
                                    include = T)]
...
### Run on sample matrix: apply function to each cell
perc_diff_values <- map_dbl(1:ncell(peru_map),
                            ~percent_different_neighbors(.))
